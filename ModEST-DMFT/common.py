...
def solve(Delta_iw, h_loc0, h_int, **solver_params):
    ...
    h_loc0_mat = {block : h_loc0[ibl].real for ibl, (block, _) in enumerate(gf_struct) }
    ...
    h_loc0     = sum(c_dag_vec[block]*h_loc0_mat[block]*c_vec[block] for block, bl_size in gf_struct)[0,0]
